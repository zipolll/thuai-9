...
inline auto buildLimitBuyMessage(const std::string& token, std::int64_t price, int quantity) -> json {
    return {{"messageType", "LIMIT_BUY"}, {"token", token}, {"price", price}, {"quantity", quantity}};
}

inline auto buildLimitSellMessage(const std::string& token, std::int64_t price, int quantity) -> json {
    return {{"messageType", "LIMIT_SELL"}, {"token", token}, {"price", price}, {"quantity", quantity}};
}

inline auto buildCancelOrderMessage(const std::string& token, std::int64_t orderId) -> json {
    return {{"messageType", "CANCEL_ORDER"}, {"token", token}, {"orderId", orderId}};
}

inline auto buildSubmitReportMessage(const std::string& token, int newsId, Prediction prediction) -> json {
    return {{"messageType", "SUBMIT_REPORT"}, {"token", token}, {"newsId", newsId}, {"prediction", predictionToString(prediction)}};
}

inline auto buildSelectStrategyMessage(const std::string& token, const std::string& cardName) -> json {
...
inline auto buildActivateSkillMessage(
    ...
    json message = {{"messageType", "ACTIVATE_SKILL"}, {"token", token}, {"skillName", skillName}};
