...
class Agent {
public:
    explicit Agent(std::string token, std::string serverUrl = "ws://localhost:14514")
    ...
    void submitReport(int newsId, Prediction prediction) {
        spdlog::debug(
            "Queueing report newsId={} prediction={}",
            newsId,
    ...
    void activateSkill(
        ...
        spdlog::debug(
            "Queueing skill activation name={} target={} variant={}",
            skillName,
            targetToken.empty() ? "none" : targetToken,
...
private:
    ...
    void send(const json& data) {
        ...
        spdlog::debug(
            "Sending messageType={} payload={}",
            data.value("messageType", std::string("UNKNOWN")),
            payload
        );
        ws_.send(payload);
    }
    ...
    void handleMessage(const std::string& raw) {
        try {
            ...
            if (msgType == "GAME_STATE") {
                ...
                spdlog::debug(
                    "Parsed game state stage={} month={} day={} tick={}/{} scoreEntries={}",
                    gameState.stage,
                ...
                spdlog::debug(
                    "Parsed market state tick={} bids={} asks={} lastPrice={} midPrice={} volume={}",
                    marketState.tick,
                    marketState.bids.size(),
                    marketState.asks.size(),
                ...
                spdlog::debug(
                    "Parsed player state mora={} frozenMora={} gold={} frozenGold={} lockedGold={} nav={} pendingOrders={} activeCards={}",
                    playerState.mora,
                    playerState.frozenMora,
                    playerState.gold,
                    playerState.frozenGold,
                    playerState.lockedGold,
                    playerState.nav,
                    playerState.pendingOrders.size(),
                    playerState.activeCards.size()
                );
                onPlayerState(playerState);
            } else if (msgType == "NEWS_BROADCAST") {
                latestNews = protocol::parseNews(data);
                spdlog::debug(
                    "Parsed news newsId={} month={} day={} publishTick={}",
                    latestNews->newsId,
                    latestNews->month,
                    latestNews->day,
                ...
                spdlog::debug(
                    "Parsed strategy options infrastructure={} riskControl={} finTech={}",
                ...
                spdlog::debug(
                    "Parsed trade tradeId={} orderId={} side={} price={} quantity={} fee={}",
                    trade.tradeId,
                    trade.orderId,
                    trade.side,
                    trade.price,
                ...
                spdlog::warn("Ignoring unknown messageType={} payload={}", msgType, raw);
            }
        } catch (const std::exception& e) {
            spdlog::error("Failed to parse inbound message: {} payload={}", e.what(), raw);
        }
    }
};
